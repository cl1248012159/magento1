(function(jQuery) {
    jQuery.fx.step.clip = function(fx) {
        ...
        fx.elem.style.clip = 'rect(' + parseInt((fx.pos * (earr[0] - sarr[0])) + sarr[0]) + 'px ' + parseInt((fx.pos * (earr[1] - sarr[1])) + sarr[1]) + 'px ' + parseInt((fx.pos * (earr[2] - sarr[2])) + sarr[2]) + 'px ' + parseInt((fx.pos * (earr[3] - sarr[3])) + sarr[3]) + 'px)';
    }
})(jQuery);
...
(function($) {
    ...
    function drawCalControl(target, opt) {
        ...
        if (opt.monthSelect) $(target).find('.jCal .monthName, .jCal .monthYear').bind('mouseover', $.extend({}, opt), function(e) {
            ...
            $(this).removeClass('monthYearHover').removeClass('monthNameHover');
        }).bind('click touchstart', $.extend({}, opt), function(e) {
            ...
            $(moSel).css('clip', 'rect(' + diffOff + 'px ' + ($(this).width() + (pad * 2)) + 'px ' + diffOff + 'px 0px)').animate({
                ...
                $(this).parent().find('.monthSelectorShadow').bind('mouseover mouseout click touchstart', function() {
            ...
            $('.jCalMo .monthSelect', e.data._target).bind('mouseover mouseout click touchstart', $.extend({}, e.data), function(e) {
                if (e.type == 'click' || e.type == 'touchstart') $(e.data._target).jCal($.extend(e.data, {
                    day: new Date($(this).attr('id').replace(/_/g, '/'))
                }));
                else $(this).toggleClass('monthSelectHover');
            });
        });
        $(target).find('.jCal .left').bind('click touchstart', $.extend({}, opt), function(e) {
            ...
            if (e.data.showMonths > 1) {
                $('.right', e.data._target).clone(true).appendTo($('.jCalMo:eq(2) .jCal', e.data._target));
        ...
        $(target).find('.jCal .right').bind('click touchstart', $.extend({}, opt), function(e) {
            if ($('.jCalMask', e.data._target).length > 0) return false;
            var mD = {
                w: 0,
                h: 0
    ...
    function drawCal(target, opt) {
        ...
        $(target).find('div[id^=' + opt.cID + 'd_]:not(.invday)').bind("mouseover mouseout click touchstart", $.extend({}, opt), function(e) {
            if ($('.jCalMask', e.data._target).length > 0) return false;
            var osDate = new Date($(this).attr('id').replace(/c[0-9]{1,}d_([0-9]{1,2})_([0-9]{1,2})_([0-9]{4})/, '$1/$2/$3'));
            if (e.data.forceWeek) osDate.setDate(osDate.getDate() + (e.data.dayOffset - osDate.getDay()));
            var sDate = new Date(osDate.getTime());
            if (e.type == 'click' || e.type=='touchstart') $('div[id*=d_]', e.data._target).stop().removeClass('selectedDay').removeClass('overDay').css('backgroundColor', '');
            for (var di = 0, ds = $(e.data._target).data('days'); di < ds; di++) {
                ...
                else if (e.type == 'click' || e.type=='touchstart' ) $(currDay).stop().addClass('selectedDay');
...
minDate.setDate(todayDate.getDate()+6);

var maxDate = new Date(todayDate);
maxDate.setDate(todayDate.getDate()+55);

jQuery(function() {
    jQuery('#calendar').jCal({
        day: new Date(),
        days: 1,
        showMonths: 3,
        monthSelect: false,
        dCheck: function (day) {
            if( minDate.getTime() < day.getTime() && day.getTime() < maxDate.getTime() ){
                return "invday";
            }
        },
        dow: ['S', 'M', 'T', 'W', 'T', 'F', 'S'],
        ml: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
        callback: function(day, days) {
            jQuery('input[data-title="Time"]').val(day.getFullYear() + '/' + (day.getMonth() + 1) + '/' + day.getDate());
            jQuery('.jCalcontainer').hide();
            jQuery('.made-to-order-time .time').html(this.ml[day.getMonth()]+' '+day.getDate()+','+day.getFullYear());
            jQuery('.made-to-order-time').show();
        }
    });
